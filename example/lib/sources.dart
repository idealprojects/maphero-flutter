...
class StyleInfo {
  ...
  final Future<void> Function(MapHeroMapController) addDetails;
...
class FullMapState extends State<FullMap> {
  ...
  static Future<void> addRaster(MapHeroMapController controller) async {
  ...
  static Future<void> addGeojsonCluster(
      MapHeroMapController controller) async {
    await controller.addSource(
        "earthquakes",
  ...
  static Future<void> addVector(MapHeroMapController controller) async {
    await controller.addSource(
        "terrain",
        const VectorSourceProperties(
          url: MapHeroStyles.demo,
        ));
  ...
  static Future<void> addImage(MapHeroMapController controller) async {
    await controller.addSource(
        "radar",
  ...
  static Future<void> addVideo(MapHeroMapController controller) async {
    await controller.addSource(
        "video",
  ...
  static Future<void> addHeatMap(MapHeroMapController controller) async {
    await controller.addSource(
  ...
  static Future<void> addDem(MapHeroMapController controller) async {
    // TODO: adapt example?
    // await controller.addSource(
    //     "dem",
  ...
  final _stylesAndLoaders = [
    const StyleInfo(
      name: "Vector",
      baseStyle: MapHeroStyles.demo,
      addDetails: addVector,
      position: CameraPosition(target: LatLng(33.3832, -118.4333), zoom: 6),
    ),
    const StyleInfo(
      name: "Default style",
      // Using the raw github file version of MapHeroStyles.DEMO here, because we need to

...
import 'package:maphero_example/attribution.dart';
import 'package:maphero_example/get_map_informations.dart';
import 'package:maphero_example/given_bounds.dart';
import 'package:maphero_example/localized_map.dart';
import 'package:maphero_example/no_location_permission_page.dart';
...
class _MapsDemoState extends State<MapsDemo> {
  ...
  Future<void> initHybridComposition() async {
    if (!kIsWeb && Platform.isAndroid) {
      ...
      if (sdkVersion >= 29) {
        MapHeroMap.useHybridComposition = true;
      } else {
        MapHeroMap.useHybridComposition = false;
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('MapHero examples')),
      body: ListView.builder(
        itemCount: _allPages.length + 1,
        itemBuilder: (_, int index) => index == _allPages.length
            ? const AboutListTile(
                applicationName: "flutter-maphero example",
              )
            : ListTile(

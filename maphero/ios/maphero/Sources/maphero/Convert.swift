...
class Convert {
    class func interpretMapHeroMapOptions(options: Any?, delegate: MapHeroMapOptionsSink) {
        guard let options = options as? [String: Any] else { return }
        if let cameraTargetBounds = options["cameraTargetBounds"] as? [[[Double]]] {
            delegate
                .setCameraTargetBounds(bounds: MHCoordinateBounds.fromArray(cameraTargetBounds[0]))
        }
        ...
        if let compassViewPosition = options["compassViewPosition"] as? UInt,
           let position = MHOrnamentPosition(rawValue: compassViewPosition)
        {
        ...
        if let attributionButtonPosition = options["attributionButtonPosition"] as? UInt,
           let position = MHOrnamentPosition(rawValue: attributionButtonPosition)
        {
    ...
    class func parseCameraUpdate(cameraUpdate: [Any], mapView: MHMapView) -> MHMapCamera? {
        ...
        case "newCameraPosition":
            ...
            return MHMapCamera.fromDict(cameraPosition, mapView: mapView)
        case "newLatLng":
        ...
        case "newLatLngBounds":
            ...
            if let padding = parseLatLngBoundsPadding(cameraUpdate) {
                return mapView.cameraThatFitsCoordinateBounds(
                    MHCoordinateBounds.fromArray(bounds),
                    edgePadding: padding
                )
            }
            
            return mapView.cameraThatFitsCoordinateBounds(MHCoordinateBounds.fromArray(bounds))
    ...
    class func getZoom(mapView: MHMapView) -> Double {
        return MHZoomLevelForAltitude(
    ...
    class func getAltitude(zoom: Double, mapView: MHMapView) -> Double {
        return MHAltitudeForZoomLevel(
            zoom,
            mapView.camera.pitch,
            mapView.camera.centerCoordinate.latitude,
            mapView.frame.size

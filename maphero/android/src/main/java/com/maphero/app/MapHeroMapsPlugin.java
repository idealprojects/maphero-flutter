...
/**
 * Plugin for controlling a set of MapHeroMap views to be shown as overlays on top of the Flutter
 * view. The overlay should be hidden during transformations or while Flutter is rendering on top of
 * the map. A Texture drawn using MapHeroMap bitmap snapshots can then be shown instead of the
 * overlay.
 */
public class MapHeroMapsPlugin implements FlutterPlugin, ActivityAware {

  static FlutterAssets flutterAssets;
  private Lifecycle lifecycle;

  public MapHeroMapsPlugin() {
    // no-op
  }
  ...
  public void onAttachedToEngine(@NonNull FlutterPluginBinding binding) {
    ...
    MethodChannel methodChannel =
        new MethodChannel(binding.getBinaryMessenger(), "plugins.flutter.io/maphero");
    methodChannel.setMethodCallHandler(new GlobalMethodHandler(binding));

    binding
        .getPlatformViewRegistry()
        .registerViewFactory(
            "plugins.flutter.io/maphero",
            new MapHeroMapFactory(

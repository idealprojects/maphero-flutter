...
///
/// To display a map, add a [MapHeroMap] widget to the widget tree.
///
/// In this plugin, the map is configured through the parameters passed to the [MapHeroMap] constructor and through the [MapHeroMapController].
/// The [MapHeroMapController] is provided at runtime by the [MapHeroMap.onMapCreated] callback.
...
///
/// The visual appearance of the map is configured through a MapHero style passed to the
/// [styleString] parameter of the [MapHeroMap] constructor.
/// The MapHero style is a JSON document following the specification at https://maplibre.org/maplibre-style-spec/.
/// The following is supposed to serve as a short introduction to the MapHero style specification:
...
/// Depending on the map's zoom level and viewport, the MapHero client libraries decide, which tiles are needed to fill the viewport and request them from the source.
///
...
///
/// Raster sources are displayed by adding a "raster" layer to the MapHero GL style.
/// Vector and GeoJson sources are displayed by adding a "line", "fill", "symbol" or "circle" layer to the MapHero GL style and specifying
...
export 'package:maphero_platform_interface/maphero_platform_interface.dart'
    show
        Annotation,
        ArgumentCallbacks,

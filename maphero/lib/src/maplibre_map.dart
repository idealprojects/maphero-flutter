...
const _globalChannel1 = MethodChannel('plugins.flutter.io/maphero');

enum AnnotationType { fill, line, circle, symbol }

typedef MapCreatedCallback = void Function(MapHeroMapController controller);
...
class MapHeroMap extends StatefulWidget {
  ...
  ///
  /// If this is set to true and the user pans/zooms/rotates the map, [MapHeroMapController] (which is a [ChangeNotifier])
  /// will notify it's listeners and you can then get the new [MapHeroMapController].cameraPosition.
  ...
  /// Set `MapHeroMap.useHybridComposition` to `false` in order use Virtual-Display
  ...
  static bool get useHybridComposition =>
      MapHeroMethodChannel.useHybridComposition;

  static set useHybridComposition(bool useHybridComposition) =>
      MapHeroMethodChannel.useHybridComposition = useHybridComposition;

  @override
  State createState() => _MapHeroMapState();
}

class _MapHeroMapState extends State<MapHeroMap> {
  final Completer<MapHeroMapController> _controller =
      Completer<MapHeroMapController>();

  late _MapHeroMapOptions _maplibreMapOptions;
  final MapHeroPlatform _mapheroPlatform = MapHeroPlatform.createInstance();
  ...
  void initState() {
    super.initState();
    _maplibreMapOptions = _MapHeroMapOptions.fromWidget(widget);
  }
  ...
  @override
  void didUpdateWidget(MapHeroMap oldWidget) {
    super.didUpdateWidget(oldWidget);
    final newOptions = _MapHeroMapOptions.fromWidget(widget);
  ...
  Future<void> onPlatformViewCreated(int id) async {
    final controller = MapHeroMapController(
      mapheroPlatform: _mapheroPlatform,
    ...
    await _mapheroPlatform.initPlatform(id);
    _controller.complete(controller);
    widget.onMapCreated?.call(controller);
  }
}

/// Configuration options for the MapHeroMap user interface.
///
/// When used to change configuration, null values will be interpreted as
/// "do not change this configuration option".
class _MapHeroMapOptions {
  _MapHeroMapOptions({
    this.compassEnabled,
  ...
  _MapHeroMapOptions.fromWidget(MapHeroMap map)
      : this(
  ...
  Map<String, dynamic> updatesMap(_MapHeroMapOptions newOptions) {
